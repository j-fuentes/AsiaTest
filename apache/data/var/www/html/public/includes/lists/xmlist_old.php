...
class xmllist extends utils {
	...
	public function row2xml($list,$total, $edt_pagina){
		...
		while ($i<=$total){
			...
			foreach($row as $key=>$value){
				...
				else $tmp .="<$key $label2 label=\"".constant("_".strtoupper($key))."\">$strValue</$key>\n";
	 ...
	 */
	function facetlist($list,$queries,$href,$TotalRecords,$rows){
			
		$SelectedFacets = $this->selFacets;
		$order = array("type","code_comarca","code_tit", "d", "ticket", "p", "districtstr", "code_info","code0","code1","code2","code3","code4","code5");
		...
		foreach($list as $key=>$value){
			...
			switch($strKey){
				...
				case "code":
					...
					$this->getCode($key, $value, $SelectedFacets,$href);
					break;
				default:					
	 ...
	 * @param unknown_type $href
	 */
	private function getCode($key, $value, $SelectedFacets,$href){
		//echo "$key\n";
		...
		if(array_key_exists("code$level",$SelectedFacets)){
			...
			if (substr($SelectedFacets["code$level"],2,3)=="401") {$nameClass[0] = _EQ; $tree[0] ="401"; } else {$nameClass[0] =_AG;}
		} else {
		...
		if($level==-1) $count=2;else $count=1;
		for($i=0;$i<$count;$i++){
			$isResult = false;
	...
	public function Filters($href){
		...
		if (count($this->selFacets)){
			...
			foreach($this->selFacets as $key=>$value){
				...
				$xmlout .= "<item $strCode href=\"".LK_PAG.$this->url_encrypt($strhref)."\">$strvalue</item>\n";
			}
			$xmlout .= "</filters>";
			return $xmlout;
		} else return "";
	...
	private function getFilterName($key,$value){
		...
		switch($key){
			...
			case "code":
				$class = new classification;
				if ($level==0){
					$tctcodes = array_merge($class->getClassLevel("0040000",_LANGUAGE),$class->getClassLevel("0040100",_LANGUAGE));
