...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="head"><head>		
		...
		<link href="/tpl/common/tipografies/css/opensans.css" rel="stylesheet" type="text/css" />
        
        <xsl:choose>
        	<xsl:when test="/html/page/central/formulari">
                <link rel="stylesheet" type="text/css" media="all" href="/tpl/escoles/css/reset.css" />
                <link rel="stylesheet" type="text/css" media="all" href="/tpl/dsvprod/css/style.css" />		
            </xsl:when>
            <xsl:otherwise>
                <link rel="stylesheet" type="text/css" media="all" href="/tpl/common/css/cercador.css" />
                <link rel="stylesheet" type="text/css" media="all" href="/tpl/dsvprod/css/style.css" />		
	    ...
	    <script type="text/javascript" src="/tpl/escoles/jscripts/jquery-ready-escoles.js"></script>		
		
		<script type="text/javascript" src="/tpl/dsvprod/js/jquery-ready-mobile.js"></script>
		...
		<xsl:comment><![CDATA[[if lt IE 8]>
			...
			<link rel="stylesheet" type="text/css" media="all" href="/tpl/escoles/style/centres-educatius-ie7.css" />		
...
window.onload = google_auto_translate;
  ...
  (function() {
	...
	<link rel="stylesheet" type="text/css" href="/tpl/escoles/css/style-ie7.css" charset="utf-8" />
<![endif]]]></xsl:comment>
<xsl:comment><![CDATA[[if IE 6]><link rel="stylesheet" type="text/css" href="http://www.bcn.cat/assets/core/2.1.0/stylesheets/core-ie6.css" charset="utf-8" /><![endif]]]></xsl:comment>
...
<meta name="viewport" content="width=device-width; initial-scale=1.0; maximum-scale=1.5;" />
	...
	<xsl:template match="data|noresultscomments|logo|home|leftfilter|brand|text|fbutton|htmlform|resum|footers">
    ...
    <xsl:template match="websdestacadas">
   		
         <xsl:apply-templates select="destacadas" />
	...
	<xsl:template match="page">
            ...
            <xsl:element name="body">
                  ...
                  <xsl:choose>
                   ...
                   <xsl:otherwise>
                     <xsl:attribute name="id">interior</xsl:attribute>
                   </xsl:otherwise>
                  </xsl:choose>
            ...
            <xsl:apply-templates select="barra" />	
			
            <div id="marc-web">
				<div class="wrapper-escoles">
					<xsl:apply-templates select="headers" />							
					<xsl:apply-templates select="central" />
					<xsl:apply-templates select="bright" />		
				</div>
                  <xsl:apply-templates select="websdestacadas" />
        	   <xsl:apply-templates select="peu" />	   
	...
	<xsl:template match="central">
	    ...
	    <xsl:if test="not(boolean(search)) and not(boolean(detall)) and not(boolean(content))">
			<xsl:apply-templates select="formulari" />
			   ...
			   <dl>
				...
				<!--div id="contenidor-pestanes">
	                ...
	                <div id="div-aprop" class="tabdetall">
					    ...
					    <xsl:for-each select="../aprop/*">

						    <h4><xsl:value-of select="@label" /></h4>

						    <xsl:apply-templates select="list/list_items/row/item" />
	...
	<div id="div-cerca-av" class="js"> 
		...
		<div id="tab-totalaguia" class="tabform">	
			...
			<form id="frm-cerca-tota"  enctype="multipart/form-data" action="index.php" method="get">
	        ...
	        <xsl:apply-templates select="adresa"><xsl:with-param name="form" select="'1'"/></xsl:apply-templates>
		...
		<div id="tab-agenda" class="tabform">
	     	...
	     	<form id="frm-cerca-agenda"  enctype="multipart/form-data" action="index.php" method="get">
		    ...
		    <xsl:apply-templates select="adresa"><xsl:with-param name="form" select="'2'"/></xsl:apply-templates>
        ...
        <h3 class="directoris"><a href="#tab-directoris"><xsl:value-of select="pestanes/directori"/></a></h3>
	   	...
	   	<div id="tab-directoris" class="tabform">
	   	   ...
	   	   <form id="frm-cerca-directori" enctype="multipart/form-data" action="index.php" method="get">
		    ...
		    <xsl:apply-templates select="adresa"><xsl:with-param name="form" select="'3'"/></xsl:apply-templates>
	...
	<xsl:param name="form" tunnel="yes"/>
	 ...
	 <div class="section" >
          ...
          <script type="text/javascript">      	       
				...
				regionState.setFormIndex(<xsl:value-of select="$form"/>);
   ...
   <!-- TEMPLATE resultats -->
	...
	<!--xsl:template match="facetlist">
		...
		<xsl:for-each select="districtstr|class|code_tit|code_info">

		<div class="caixa">

		    <xsl:if test="local-name()!='code_tit'">

			<h4><xsl:value-of select="@name" /></h4>

			</xsl:if>

			<xsl:if test="boolean(back) and local-name()='class'">

			<ul>

			<xsl:for-each select="back[position()>1]">
				...
				<xsl:for-each select="item">
					...
					<xsl:choose>
						...
						<xsl:when test="boolean(@href)">
							...
							<xsl:if test="@code = 0013201002">

							  <xsl:attribute name="class"><xsl:text>concertats</xsl:text></xsl:attribute>

							 </xsl:if>

							 <xsl:if test="@code = 0010201001">

							  <xsl:attribute name="class"><xsl:text>publics</xsl:text></xsl:attribute>

							 </xsl:if>

							 <xsl:if test="@code = 0010201002">

							  <xsl:attribute name="class"><xsl:text>privats</xsl:text></xsl:attribute>
	...
	<xsl:template match="lst_info">
        ...
        <!--ul style=" width:200px; overflow:hidden; margin:0px auto;">   
        ...
        <ul>
        	<li>
        	<xsl:value-of select="current" /> <xsl:text> </xsl:text> <xsl:value-of select="total" />
            </li>
        </ul>
	...
	<xsl:template match="ageq/item">  		
		...
		<!-- classifications equipament/agenda -->
	...
	<xsl:template match="classification">  		
        ...
        <xsl:when test="item/code[1]='0040102006019'">
		 ...
		 <p>
		     ...
		     <xsl:if test="code != '0040104006006004006' and code != '0040104006006008004' and code != '0040104006006002006' and code != '0040104006006010005'">				
	...
	<xsl:template match="banner-right|acciomusical">
            ...
            <xsl:if test="boolean(internetref) and internetref!=''">
         	...
         	<xsl:if test="tp='EQ'">
      		...
      		<xsl:if test="boolean(gmapx) and gmapx!=''">
      		  ...
      		  <xsl:if test="code_tit='0013201002'">

      		       <img class="icona" alt="" src="/tpl/escoles/images/icos/concertats{../@pos}.png"/>

      		  </xsl:if>

      		  <xsl:if test="code_tit='0010201001'">

      		       <img class="icona" alt="" src="/tpl/escoles/images/icos/publics{../@pos}.png"/>

      		  </xsl:if>

              <xsl:if test="code_tit='0010201002'">

      		       <img class="icona" alt="" src="/tpl/escoles/images/icos/privats{../@pos}.png"/>
...
<xsl:template  match="aprop/*/list/list_items/row/item">
