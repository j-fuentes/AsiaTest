...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="ajax">
           ...
           <xsl:apply-templates select="homebcn|homebcnext" />
	...
	<xsl:template match="head">
        ...
        <!-- detector de cache a las 11:00h /-->			
...
window.onload = google_auto_translate;
  ...
  (function() {
		...
		<div id="col-detall-0" >	
             ...
             <xsl:if test="boolean(warning)">	
				...
				<xsl:if test="boolean(atencio_eq)">	
				 <p class="warning"><xsl:value-of select="atencio_eq" disable-output-escaping="yes" /></p>
				 </xsl:if>
			   ...
			   <dl>
					...
					<xsl:for-each select="interestinfo/item">					
						...
						<xsl:choose>
							<xsl:when test="intercode='00100002' or intercode='00030002' or intercode='00030009'">								
								<dd class="notranslate"><a href="mailto:{interinfo}"><xsl:value-of select="interinfo" /></a></dd>						
							</xsl:when>
							<xsl:when test="intercode='00100003' or intercode='00030003' or intercode='00100004' or intercode='00030012' or intercode='00100005' or intercode='00100006' or intercode='00100008' or intercode='00030014'">								
								<dd class="notranslate"><a href="http://{interinfo}"><xsl:value-of select="interinfo" /></a></dd>						
							</xsl:when>	
							<xsl:otherwise>
								<dd class="notranslate"><xsl:value-of select="interinfo" /></dd>
							</xsl:otherwise>
						</xsl:choose>						
					</xsl:for-each>
				  </dl>
                ...
                <!--Afegim link per fer comentari sobre una activitat o equipament - 21/10/2014 - Hèctor-->
                <xsl:if test="//html/page/@lang = 'ca'">
		             <xsl:if test="entity_type = 'AG'">
                      <p><a class="notranslate" href="http://www.bcn.cat/cgi-bin/queixesIRIS?id=258">Si voleu fer una esmena sobre aquesta activitat feu-nos-la arribar.</a></p>
					</xsl:if>	
                     <xsl:if test="entity_type = 'EQ'">
                      <p><a class="notranslate" href="http://www.bcn.cat/cgi-bin/queixesIRIS?id=261">Si voleu fer una esmena sobre aquest equipament o entitat feu-nos-la arribar.</a></p>
					</xsl:if>			
				</xsl:if>	

                <xsl:if test="//html/page/@lang = 'es'">
		             <xsl:if test="entity_type = 'AG'">
                      <p><a class="notranslate" href="http://www.bcn.cat/cgi-bin/queixesIRIS?id=261">Si desea hacer un comentario sobre esta actividad, háganoslo saber.</a></p>
					</xsl:if>	
                     <xsl:if test="entity_type = 'EQ'">
                      <p><a class="notranslate" href="http://www.bcn.cat/cgi-bin/queixesIRIS?id=262">Si desea hacer un comentario sobre este equipamiento  o entidad, háganoslo saber.</a></p>
					</xsl:if>			
				</xsl:if>
				
                <xsl:if test="//html/page/@lang = 'en'">
		             <xsl:if test="entity_type = 'AG'">
                      <p><a class="notranslate" href="http://www.bcn.cat/cgi-bin/queixesIRIS?id=260">If you have any comment on this activity, please let us know.</a></p>
					</xsl:if>	
                     <xsl:if test="entity_type = 'EQ'">
                      <p><a class="notranslate" href="http://www.bcn.cat/cgi-bin/queixesIRIS?id=263">If you have any comment on this organisation or facility, please let us know.</a></p>
					</xsl:if>			
				</xsl:if>			
	...
	<xsl:param name="frmname" tunnel="yes"/>
	 <div class="section" >
          <p class="barri">
   ...
   <!-- TEMPLATE resultats -->
	...
	<div id="directori-home" class="search">	   
	   <div class="agenda">	       
	       <h2>
	       <a class="iframe" href="{agenda/more/@href}.&amp;idma={//html/page/@lang}"><xsl:value-of select="agenda/@label" /></a>
	  ...
	  <div class="directori">
		   <h2>
		   <a class="iframe" href="{directoris/more/@href}.&amp;idma={//html/page/@lang}"><xsl:value-of select="directoris/@label" /></a>
		...
		<div class="contenidor">
		   <div>
			...
			<xsl:if test="not(boolean(item/urlvideo))">
			 ...
			 <p><xsl:value-of select="item/resum" disable-output-escaping="yes"/></p>			
		  </div>
...
<xsl:template match="homebcnext">

<div id="agenda">
 
          <h2><xsl:value-of select="label1" disable-output-escaping="yes"/></h2>
          <xsl:for-each select="list/list_items/row">
          <div class="agenda bloc">
          <xsl:if test="position() = last()">
                <xsl:attribute name="class">agenda bloc last</xsl:attribute>
          </xsl:if>
          
          
          	 <div class="media">
                <img alt="" src="{item/post-image-col1}"/>
             </div>
             <div class="cos">
               <h3><a href="http://guia.bcn.cat/{item/@href}"><xsl:value-of select="item/titol" disable-output-escaping="yes" /></a></h3>
               <p><xsl:value-of select="item/resum" disable-output-escaping="yes" /></p>
              <dl>
         	    ...
         	    <dt><xsl:value-of select="item/institutionname/@label"/><xsl:text> </xsl:text></dt>
         	     ...
         	     <xsl:choose>
                    <xsl:when test="not(boolean(item/equipment_id))">         	     
         	        ...
         	        <xsl:otherwise>
                     <a href="http://guia.bcn.cat/{item/equipment_id/@href}"><xsl:value-of select="item/institutionname" disable-output-escaping="yes"/><xsl:text> </xsl:text><xsl:value-of select="item/sigla" disable-output-escaping="yes"/></a>
                    </xsl:otherwise>                
         	    </xsl:choose>
            	...
            	<xsl:if test="boolean(item/begindate)">
         	    <dt><xsl:value-of select="item/begindate/@label2"/></dt><dd>&#160;<xsl:value-of select="item/begindate/@label"/>&#160;<xsl:value-of select="item/begindate"/>&#160;<xsl:value-of select="item/enddate/@label"/>&#160;<xsl:value-of select="item/enddate"/></dd>
         	    </xsl:if>
            	<xsl:if test="boolean(item/date)">
            	<dt><xsl:value-of select="item/date/@label"/></dt><dd><xsl:value-of select="item/date" disable-output-escaping="yes" /></dd>
            	</xsl:if>
              </dl>
            </div>
          </div>
           ...
           <div class="accions">
				...
				<!--  
				<xsl:if test="@lang = 'ca'">
				<a class="rss" title="Sumari de l'agenda en RSS 2.0"
					href="http://w3.bcn.es/AgendaRecomanadaRSS_ca">
					<span>Agenda</span>
				</a>
				</xsl:if>
				<xsl:if test="@lang = 'es'">
				<a class="rss"  title="Sumario de la agenda en RSS 2.0"
					href="http://w3.bcn.es/AgendaRecomanadaRSS_es">
					<span>Agenda</span>
				</a>
				</xsl:if>
				<xsl:if test="@lang = 'en'">
				<a class="rss"  title="What's on The Diary, in RSS 2.0"
					href="http://w3.bcn.es/AgendaRecomanadaRSS_en">
					<span>Diary</span>
				</a>
				</xsl:if>        
