...
switch(theme_get_setting('color')) {
  case "verd":
    drupal_add_css(path_to_theme() . '/css/colors/verd.css', array('group' => CSS_THEME) );
  break;
  case "blau":
    drupal_add_css(path_to_theme() . '/css/colors/blau.css', array('group' => CSS_THEME) );
    break;
  case "rosa":
    drupal_add_css(path_to_theme() . '/css/colors/rosa.css', array('group' => CSS_THEME) );
  break;
  default:
    drupal_add_css(path_to_theme() . '/css/colors/blau.css', array('group' => CSS_THEME) );
  break;
...
function bcn_preprocess_html(&$vars) {
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = bcn_id_safe('page-' . $path);
    $vars['classes_array'][] = bcn_id_safe('section-' . $section);
  ...
  if (theme_get_setting('basic_ie_enabled')) {
   ...
   if (drupal_match_path(drupal_get_path_alias(), 'agenda')) {
	 $vars['theme_hook_suggestions'][] = 'html__agenda';
   }
   if (drupal_match_path(drupal_get_path_alias(), 'agenda/*')) {
	 $vars['theme_hook_suggestions'][] = 'html__agenda';
   }
   
   if (drupal_match_path(drupal_get_path_alias(), 'equipaments')) {
	 $vars['theme_hook_suggestions'][] = 'html__equipaments';
   }
   if (drupal_match_path(drupal_get_path_alias(), 'equipaments/*')) {
	 $vars['theme_hook_suggestions'][] = 'html__equipaments';
   }
   
   if (drupal_match_path(drupal_get_path_alias(), 'cursos-i-tallers')) {
	 $vars['theme_hook_suggestions'][] = 'html__cursos';
   }
   if (drupal_match_path(drupal_get_path_alias(), 'cursos-i-tallers/*')) {
	 $vars['theme_hook_suggestions'][] = 'html__cursos';
   }
...
function bcn_preprocess_page(&$vars, $hook) {
  ...
  if (isset($vars['node']->type) && isset($vars['node']->nid)) {
   ...
   if (drupal_match_path(drupal_get_path_alias(), 'agenda')) {
	 $vars['theme_hook_suggestions'][] = 'page__agenda';
   }
   if (drupal_match_path(drupal_get_path_alias(), 'agenda/*')) {
	 $vars['theme_hook_suggestions'][] = 'page__agenda';
   }
   ...
   if (drupal_match_path(drupal_get_path_alias(), 'equipaments')) {
	 $vars['theme_hook_suggestions'][] = 'page__equipaments';
   }
   if (drupal_match_path(drupal_get_path_alias(), 'equipaments/*')) {
	 $vars['theme_hook_suggestions'][] = 'page__equipaments';
   }
   ...
   if (drupal_match_path(drupal_get_path_alias(), 'cursos-i-tallers')) {
	 $vars['theme_hook_suggestions'][] = 'page__cursos';
   }
   if (drupal_match_path(drupal_get_path_alias(), 'cursos-i-tallers/*')) {
	 $vars['theme_hook_suggestions'][] = 'page__cursos';
   }
...
function bcn_preprocess_node(&$vars) {
...
function bcn_preprocess_block(&$vars, $hook) {
  // Add a striping class.
...
function bcn_breadcrumb($variables) {

  $crumbs = "";
  $breadcrumb = $variables['breadcrumb'];

  variable_set('bcn_breadcrumb_home', t('Home'));
   

  if (!empty($breadcrumb)) {
      $crumbs = '<div id="breadcrumb"><ul class="breadcrumbs grid-12">';

      foreach ($breadcrumb as $i => $value) {
      $classExtra = "";
      if ($i==0) {
        $classExtra = ' class="first"';
      }
           $crumbs .= '<li' . $classExtra . '>' . $value . '</li>';
     }
 ...
 *
 */
function bcn_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = bcn_id_safe($element['#title']);
 ...
 */
function bcn_preprocess_menu_local_task(&$variables) {
 ...
 */
function bcn_menu_local_tasks(&$variables) {
  $output = '';
